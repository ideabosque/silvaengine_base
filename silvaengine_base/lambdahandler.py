...
class LambdaBase(object):
    ...
    def get_item(cls, table, **key):
        try:
            ...
            response = cls.dynamodb.Table(table_name).get_item(Key=key)
        except ClientError:
            raise
        else:
    ...
    def get_setting(cls, setting_id):
        ...
        try:
            ...
            assert (
                response["Count"] > 0
            ), "Cannot find values with the setting_id({0})".format(setting_id)
            return dict(
                (item["variable"], item["value"]) for item in response.get("Items")
            )
    ...
    def get_function(cls, endpoint_id, funct, api_key="#####", method=None):
        ...
        if endpoint_id != "0":
            endpoint = cls.get_item("endpoints", **{"endpoint_id": endpoint_id})
            endpoint_id = endpoint_id if endpoint.get("special_connection") else "1"

        connection = cls.get_item(
            "connections", **{"endpoint_id": endpoint_id, "api_key": api_key}
        )
        ...
        assert (
            ...
            funct=funct, endpoint_id=endpoint_id, api_key=api_key
        )

        function = cls.get_item(
            "functions",
            **{
        ...
        setting = dict(
            cls.get_setting(function["config"].get("setting")),
            **cls.get_setting(functs[0].get("setting"))
        )
...
class Resources(LambdaBase):
    ...
    def handle(self, event, context):
        ...
        try:
            ...
            assert (
                function is not None and setting is not None
            ) and area == function.get(
                "area"
            ...
            return {
                ...
                "body": (json.dumps({"error": "{0}".format(log)}, indent=4)),
            }
            ...
            if queue_name is not None:
                try:
                    ...
                    if len(messages) > 0:
                        ...
                        self.logger.info(
                            "endpoint_id: {endpoint_id}, funct: {funct}".format(
                                endpoint_id=endpoint_id, funct=funct
                            )
                        )
                ...
                self.logger.info(
                    "queue_url: {queue_url}, processed_messages: {processed_messages}, total_messages: {total_messages}".format(
                        queue_url=queue_url,
                ...
                self.logger.info(
                    "endpoint: {endpoint_id}, funct: {funct}".format(
                        endpoint_id=endpoint_id, funct=funct
                    )
                )
